...
#include "../../../include/ui/samples/DragImage.h"
...
// MyFrame
wxIMPLEMENT_DYNAMIC_CLASS(DragImageFrame, wxFrame);

wxBEGIN_EVENT_TABLE(DragImageFrame, wxFrame)
                EVT_MENU    (wxID_ABOUT, DragImageFrame::OnAbout)
                EVT_MENU    (wxID_EXIT, DragImageFrame::OnQuit)
wxEND_EVENT_TABLE()

DragImageFrame::DragImageFrame()
...
void DragImageFrame::OnQuit(wxCommandEvent &WXUNUSED(event)) {
    Close(true);
}

void DragImageFrame::OnAbout(wxCommandEvent &WXUNUSED(event)) {
...
bool MyApp::OnInit() {
    ...
    DragImageFrame *frame = new DragImageFrame();
